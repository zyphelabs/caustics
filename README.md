...
### The `caustics_default` Attribute

For fields that should be auto-generated by the database but are not auto-increment primary keys, you can use the `#[sea_orm(caustics_default)]` attribute to exclude them from the `create` method:
...
pub struct Model {
    ...
    pub id: Uuid,            // Excluded from create() - database generates UUID
    pub email: String,
    pub name: String,
    pub created_at: DateTime<FixedOffset>,
    pub updated_at: DateTime<FixedOffset>,
...
- UUID primary keys that are generated by the application/database
- Timestamp fields that are automatically set by the database
- Any field that should be excluded from manual creation
